...
export default function ShtCarSeatMap({
        ...
        if (isOpen) {
            ...
            const loadData = async () => {
                try {
                    ...
                    const rows = (data as SeatReservation[] || []);
                    console.log(`Loaded SHT car data: rows=${rows.length}`);
                    setAllData(rows);
                } catch (error) {
    ...
    const handleRequestClose = () => {
        if (!readOnly && preventCloseWithoutSave) {
            alert('좌석을 선택하고 "좌석 선택 완료"를 눌러야만 닫을 수 있습니다.');
            return;
        }
        onClose();
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between p-6 border-b">
                    ...
                    {!(preventCloseWithoutSave && !readOnly) && (
