    ...
    const loadExistingReservation = async (userId: string) => {
        try {
            ...
            setForm(prev => ({
                ...
                // 공항명은 ra_airport_location 컬럼으로 소스 변경 (기존 ra_airport_name 보조)
                airportName: first.ra_airport_location || first.ra_airport_name || '',
                pickupAirportName: isPickup ? (first.ra_airport_location || first.ra_airport_name || '') : '',
                sendingAirportName: !isPickup ? (first.ra_airport_location || first.ra_airport_name || '') : '',
                ...
                // 픽업/하차 위치는 이제 accommodation_info 컬럼에 저장됨
                pickupLocation: isPickup ? (first.accommodation_info || first.ra_airport_location || '') : '',
                ...
                sendingLocation: isPickup ? '' : (first.accommodation_info || first.ra_airport_location || ''),
        ...
        try {
            ...
            if (isEditMode && existingReservationId) {
                ...
                const insertDetail = async (
                    ...
                    const carCount = 1;
                    // 변경: 공항명은 ra_airport_location에 저장, 실제 픽업/하차 위치는 accommodation_info에 저장
                    const { data, error } = await supabase.from('reservation_airport').insert({
                        ...
                        ra_airport_location: airportName, // 공항명 저장
                        accommodation_info: location, // 하차/승차 위치 저장
            ...
            const insertAirportDetail = async (
                ...
                // 변경: 공항명은 ra_airport_location에 저장, 실제 위치는 accommodation_info에 저장
                const airportReservation = {
                    reservation_id: newReservation.re_id,
                    airport_price_code: code,
                    ra_airport_name: airportName,
                    ra_airport_location: airportName, // 공항명 저장
                    accommodation_info: location, // 승하차 위치 저장
                    ra_flight_number: flightNum,
